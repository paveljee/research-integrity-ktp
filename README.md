> [!CAUTION]
> This project, including a substantial portion of its Python codebase and the entirety of this README file, was significantly developed by an AI agent (Jules). While developed with diligence based on user specifications, AI-generated code and documentation may contain unforeseen errors, inefficiencies, or require further validation for critical applications. Users should exercise due diligence and independently verify all aspects of this project before deployment or use in production environments. For human-curated summaries of changes and known issues, please consult [GitHub Releases page for this repository](https://github.com/pvzhelnov/research-integrity-ktp/releases). This README document, in its entirety, was generated by the AI agent Jules.
...
**AI Contribution Alert**

Finally, the user requested an explicit alert at the beginning of the README to inform users about the significant level of AI contribution to the project and the AI generation of the README itself.

*   **User Request**: Add a GitHub-style `[!CAUTION]` alert at the start of the README, assessing the AI's contribution and stating that the README was AI-generated.
*   **Jules' Actions**:
    1.  *Review and Assessment*: Jules reviewed the session log (Section 6) to form an impression of the AI's contribution to both the codebase and documentation, concluding it was very significant for the code and total for the README.
    2.  *Alert Drafting*: A `[!CAUTION]` alert was drafted to reflect this assessment, advise users about potential risks of AI-generated content, and recommend due diligence. It explicitly stated that the README was entirely AI-generated by Jules.
    3.  *README Update*: The drafted alert was prepended to the beginning of the `README.md` file.

**Refinement of AI Contribution Alert**

Following the addition of the `[!CAUTION]` alert, the user requested a refinement to guide users towards human-written release notes.

*   **User Request**: Modify the `[!CAUTION]` alert to advise users to consult GitHub Releases for human-curated summaries.
*   **Jules' Actions**:
    1.  *Alert Modification*: Jules drafted an additional sentence for the alert: "For human-curated summaries of changes and known issues, please consult the GitHub Releases page for this repository."
    2.  *README Update*: This sentence was integrated into the existing `[!CAUTION]` block at the beginning of `README.md`, placed before the final statement about the README's AI authorship.

**Linking to GitHub Releases in Alert**

As a further refinement to the `[!CAUTION]` alert, the user specified that the reference to the GitHub Releases page should be an actual relative Markdown link.

*   **User Request**: Change the plain text "the GitHub Releases page for this repository" within the `[!CAUTION]` alert to a relative Markdown link `[GitHub Releases page for this repository](releases)`.
*   **Jules' Actions**:
    1.  *Link Implementation*: Jules modified the alert text, converting the specified phrase into the relative Markdown link `[GitHub Releases page for this repository](releases)`.
    2.  *README Update*: The `[!CAUTION]` block at the beginning of `README.md` was updated with this linked text.
